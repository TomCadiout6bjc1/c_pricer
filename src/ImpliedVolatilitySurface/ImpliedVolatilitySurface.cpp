...
////////////////////////////// Construction Sanity Check Methods //////////////////////////////////////
...
{
	for (size_t mat_idx = 0; mat_idx < maturity_size(); ++mat_idx)
		for (size_t strike_idx = 0; strike_idx < strike_size(); ++strike_idx)
...
{
    // I don't want to write everything in this method
    // It re-uses the private smile_implied_Volatility by using the linear interpolation in variance or extrapolation
...
double ImpliedVolatilitySurface::smile_implied_volatility(const size_t& maturity_index, const double& strike) const
{
	...
	else
	{
		...
		if (strike >= _strikes[strike_size() - 1])
		{
			...
			double deriv_right = 3. * _alpha_coefficients[maturity_index][strike_size() - 2] * _delta_strikes[strike_size() - 2] * _delta_strikes[strike_size() - 2] + 2. * _beta_coefficients[maturity_index][strike_size() - 2] * _delta_strikes[strike_size() - 2] + _gamma_coefficients[maturity_index][strike_size() - 2];
			implied_vol = _market_implied_volatilities[maturity_index][strike_size() - 1] + deriv_right * (strike - _strikes[strike_size() - 1]);
		}
		else
			// interpolation case	
		{
